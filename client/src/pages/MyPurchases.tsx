                    ...
                    <div key={idx} className="flex items-center gap-4">
                      ...
                      <div>
                        {item.product?.category === 'Notes' && item.product?.fileUrl ? (
                          <Button variant="outline" size="sm" className="rounded-full gap-2 text-primary-600 border-primary-200 hover:bg-primary-50" onClick={() => window.open(`${import.meta.env.VITE_API_URL || import.meta.env.VITE_API_URL}${item.product.fileUrl}`, '_blank')}>
