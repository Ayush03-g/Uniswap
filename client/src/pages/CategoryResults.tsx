...
import { PackageOpen, MapPin, PlusCircle, Heart, ShoppingCart } from "lucide-react"
...
export function CategoryResults() {
  ...
  const { data: products = [], isLoading, isError } = useGetProductsByCategoryQuery(categoryName || "", { skip: !categoryName })
  ...
  return (
    <div className="container mx-auto px-4 py-8 flex flex-col min-h-[calc(100vh-4rem)]">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Explore all items in {categoryName}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-red-500">
          <p>Failed to load category results. Please check if the server is running.</p>
        </div>
      ) : products.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
          ...
          <h2 className="text-2xl font-bold tracking-tight mb-3 text-white">No items are currently available in this category.</h2>
          <p className="text-muted-foreground max-w-md mb-8">
            Be the first student to list an item.
          </p>
          <Button size="lg" className="rounded-2xl gap-2 font-semibold shadow-md bg-gradient-to-r from-[#6C3BFF] to-[#8B5CF6] hover:from-[#8B5CF6] hover:to-[#A855F7] text-white border-0" onClick={() => navigate("/sell")}>
